...
$sql="select s.model as model,s.price as price,s.image as image,s.observation,c.descripcion as color,b.description as brand, date_format(s.initial_date,'%d-%m-%Y') as initial_date from sneakers s inner join brands b on b.id_brand=s.id_brands inner join colors c on s.id_colors=c.id_colors order by s.initial_date ";
...
<li>
    <div class="box">
        <figure>
            ...
            <figcaption>
            <h3> <?php echo $row['brand'] .' ' . $row['model'] .' </br>' . $row['color'] ?></h3></figcaption>
            <p><?php echo $row['price']?></p> 
            <time><?php echo $row['initial_date']?></time>
